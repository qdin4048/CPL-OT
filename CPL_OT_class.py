...
class SSL(nn.Module):
    def __init__(
        ...
        self.word_embedding = nn.Parameter(primal_X_0, requires_grad = False)
        self.gamma = gamma
        ...
        self.kernel_gate = nn.Parameter(self.glorot([self.input_dim, self.input_dim]).float(), requires_grad = True)
        self.bias_gate = nn.Parameter(torch.zeros([self.input_dim]).float(), requires_grad = True)
        self.Weight_1 = nn.Parameter(self.glorot([self.input_dim, self.input_dim]).float(), requires_grad = True)
        self.Weight_2 = nn.Parameter(self.glorot([self.input_dim, self.input_dim]).float(), requires_grad = True)
        ...
        self.M = self.get_sparse_tensor(self.e, self.KG, self.device)

        self.ent_r = ent_r


    def build(self, inputs):
        """word-embedding-consisted entity embeddings layer"""
        word_embedding = self.word_embedding
        word_embedding = F.normalize(word_embedding, 2, -1)

        """Incorporating neighborhood relation embeddings"""
        ...
        # HGCN_layer1
        gcn_layer1 = self.add_dense_layer(node_embedding_nr, self.M, self.relu, dropout=0.0, dense = self.Weight_1)
        hgcn_layer1 = self.highway(node_embedding_nr, gcn_layer1, self.input_dim, self.kernel_gate, self.bias_gate)

        # HGCN_layer2
        gcn_layer2 = self.add_dense_layer(hgcn_layer1, self.M, self.relu, dropout=0.0, dense = self.Weight_2)
    ...
    def add_dense_layer(self, inlayer, M, act_func, dropout, dense):
        inlayer = F.dropout(inlayer, p = dropout)
